...
def get_words_to_point(word_list):
    ...
    for i in word_list:
        for letter in i:
            score_table[i]=score_table.get(i,0)+SCRABBLE_LETTER_VALUES[lettter]

def get_max(dictionary):
    max_value=dictionary[0]
    for i in dictionary:
        if max_value < dictionary[i]:
            max_key=i
    return max_key

def get_valid_word(dictionary, hand):
    dic={}
    for word in dictionary:
        if is_valid_word(word, hand, points_dict):
            dic[word]=dictionary[word]
    return dic

def pick_best_word(hand, points_dict):
    word_possible=get_valid_word(points_dict, hand)
    best_word=get_max(word_possible)
    print best_word



points_dict=get_words_to_point(load_words())
